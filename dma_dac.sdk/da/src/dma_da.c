...
#define BEGIN_FREQ 1000
#define END_FREQ 310000
#define STEP_FREQ 993
 ...
 */
//#define MAX_PKT_LEN		   1024	/* must be bigger than 1024, or FIFO will be empty */
int PKT_LEN = 128800;
...
volatile int rst_flag;

/*
 * DMA buffer to MM2S
 */
u8 DmaTxBuffer[fk_num] __attribute__ ((aligned(64)));
/*
 * wave buffer
 */
u8 WaveBuffer[fk_num] __attribute__ ((aligned(64)));
...
int XAxiDma_DAC(u16 DeviceId)
{
	...
	int wave_sel = 0, freq = BEGIN_FREQ;
	...
	while(1)
	{
		while (freq < END_FREQ)
		{
			...
			PKT_LEN = floor(fk_num / freq);

			GetSinWave(PKT_LEN, MAX_AMP_VAL, AMP_VAL, WaveBuffer) ;
			memcpy(DmaTxBuffer, WaveBuffer, PKT_LEN) ;
			Xil_DCacheFlushRange((UINTPTR)DmaTxBuffer, PKT_LEN);
//			AD9708_SEND_mWriteReg(AD9708_BASE, AD9708_START, 1) ;
			XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) DmaTxBuffer, PKT_LEN, XAXIDMA_DMA_TO_DEVICE);
			sleep(2/freq);
		}
	}
...
void GpioHandler(void *CallbackRef)
{
	...
	Clean_val = XGpioPs_IntrGetStatusPin(GpioInstancePtr, 51);
